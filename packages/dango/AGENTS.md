...
- **`request.headers` accepts two forms**: a static record `{ Name: expr }` where each value is an expression, OR a single expression that evaluates to a `{ Name: value }` object. Use the single-expression form when header names are dynamic (e.g. user-supplied auth headers in `builtin:dandanplay`).
