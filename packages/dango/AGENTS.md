...
- **Singleton-unwrap**: JSONata unwraps single-element projection results. Wrap the expression in `[...]` to force an array. This bites intermediate values as much as `output`: an `extract` or `assign` feeding `forEach.in` throws `forEach.in must evaluate to an array` the day upstream returns exactly one row.
...
- **`forEach` failure handling** is a host policy (RunOption), not a manifest field. By default a forEach fails fast: the first iteration failure cancels the in-flight siblings (via an `AbortController` linked to the run's signal) and rejects. The `continueOnError` RunOption (threaded through `ManifestRunner` per call) instead skips failed iterations and returns partial results; a genuine abort always propagates. Hosts typically set it only on `runDanmaku`.
...
- **`identityFields` is required** and declares which `configSchema` properties partition the manifest's content namespace, i.e. which configValues make the same provider id mean different content on different deployments. Empty `[]` = fixed-site source with one namespace (the manifest id alone): `bilibili`, `tencent`. A self-hostable / multi-instance source lists the deployment-varying field(s), e.g. `dandanplay` declares `["baseUrl"]` because a self-hosted DanDanPlay-compatible server has its own private id space. Each entry must name a declared top-level `configSchema` property (the schema rejects unknown entries and a non-empty list with no `configSchema`). Omitting the field entirely is an authoring error, never a silent default. The host reads it (via `ManifestRunner.identityFields()`), restricts a season's configValues to those fields, normalizes, and hashes them into a stable namespaceKey; the engine never computes the key. A later manifest version may change the list (e.g. `[]` -> `["baseUrl"]`).
