...
async function rawFetch(
  ...
  // A browser-ish UA defeats bilibili's "is this a bot" sniff. A browser host
  // gets this for free; Node does not.
...
const fetcher = async (
  ...
  // a browser host's FetchLike re-injects them via a request-rewrite
  // mechanism. From Node there's no such restriction, so we can merge them
  // straight into the outgoing headers.
...
function ddpCreds(): { appId: string; appSecret: string } {
  const appId = process.env.DDP_APP_ID
  const appSecret = process.env.DDP_APP_SECRET
  if (!appId || !appSecret) {
    console.error(
      'ddp smoke needs DDP_APP_ID and DDP_APP_SECRET env vars (official api.dandanplay.net open API).'
    )
    process.exit(2)
  }
...
const pipelines: Record<string, Pipeline<unknown, unknown>> = {
  ddp: {
    ...
    episodeInputs: (s) => {
      return {
        bangumiId: (s as DdpSearch).providerIds.bangumiId,
        ...ddpCreds(),
      }
    },
    danmakuInputs: (e) => {
      return {
        episodeId: (e as DdpEpisode).providerIds.episodeId,
        ...ddpCreds(),
      }
    },
    describeSearch: (s) => {
  ...
  if (source === 'bilibili') {
    console.error(
      `\n[smoke:${source}] note: bilibili's anti-bot wall rejects scripted` +
        ...
        ' fixture tests; live verification happens in a real browser context,' +
        ' where those values are filled in automatically.'
    )
  }
  process.exit(1)
})
