...
describe('ManifestRunner', () => {
  ...
  it('runs the search pipeline with bound options', async () => {
    const animes = [
      { animeId: 1, animeTitle: 'Frieren' },
      { animeId: 2, animeTitle: 'Bocchi' },
    ]
...
describe('ManifestRunner.runLoginProbe', () => {
  it('returns null when the manifest declares no loginProbe pipeline', async () => {
    ...
    expect(await runner.runLoginProbe<boolean>()).toBeNull()
  })
  ...
  it('threads per-call inputs into the probe pipeline', async () => {
    ...
    expect(await runner.runLoginProbe<boolean>({ accountId: '42' })).toBe(true)
    expect(calls[0].url).toBe('https://api.example.com/u/42')
  })
})

/**
 * Covers the generic http-step `cache`: a cache-enabled step reuses its
 * extract bag across runs of the same ManifestRunner instance until the TTL
 * expires, while non-cached steps fetch every time. Asserts fetch counts and
 * that expiry falls through to a fresh fetch.
 */
describe('http step cache', () => {
        ...
        steps: [
          {
            type: 'http',
            id: 'keys',
            request: { method: 'GET', url: "'https://api.example.com/keys'" },
            extract: { token: 'data.token' },
            cache: { key: 'keys', ttlSeconds: 60 },
          },
          {
            type: 'http',
            id: 'search',
  ...
  it('fetches the cached step once across two runSearch calls', async () => {
    const { fetcher, calls } = mockFetcher({
      'https://api.example.com/keys': {
        body: JSON.stringify({ data: { token: 'tk' } }),
      },
      'https://api.example.com/search': {
        body: JSON.stringify({ data: 'ok' }),
      },
    })
    const runner = new ManifestRunner(cacheManifest(), { fetcher })

    await runner.runSearch({ q: 'a' })
    await runner.runSearch({ q: 'b' })

    const keysCalls = calls.filter(
      (c) => c.url === 'https://api.example.com/keys'
    )
    const searchCalls = calls.filter(
      (c) => c.url.split('?')[0] === 'https://api.example.com/search'
    )
    expect(keysCalls).toHaveLength(1)
    expect(searchCalls).toHaveLength(2)
  })

  it('re-fetches the cached step once the TTL expires', async () => {
    setSystemTime(new Date('2026-05-11T12:00:00Z'))
    const { fetcher, calls } = mockFetcher({
      'https://api.example.com/keys': {
        body: JSON.stringify({ data: { token: 'tk' } }),
      },
    ...
    await runner.runSearch({ q: 'a' })
    setSystemTime(new Date('2026-05-11T12:02:00Z'))
    await runner.runSearch({ q: 'b' })

    const keysCalls = calls.filter(
      (c) => c.url === 'https://api.example.com/keys'
    )
    expect(keysCalls).toHaveLength(2)
    setSystemTime()
  })
  ...
  it('fetches every time when the step declares no cache', async () => {
    ...
    const runner = new ManifestRunner(noCacheManifest, { fetcher })
...
describe('JsonataEvaluator', () => {
  it('re-evaluates the same expression against different contexts', async () => {
  ...
  it('keeps evaluating correctly past maxCacheSize distinct expressions', async () => {
  ...
  it('keeps evaluating correctly after clear()', async () => {
